...
function loadProfile() {
  try {
    const p = localStorage.getItem("restos_profile");
    return p ? JSON.parse(p) : null;
  } catch { return null; }
}

function saveProfile(profile) {
  localStorage.setItem("restos_profile", JSON.stringify(profile));
}

function applyColor(color) {
  document.documentElement.style.setProperty("--orange",  color);
  document.documentElement.style.setProperty("--orange2", color + "cc");
}

function applyProfile(profile) {
  if (!profile) return;
  applyColor(profile.color);
  document.getElementById("appTitle").textContent  = profile.name ? `Les Restos de ${profile.name}` : "Mes Restos";
  document.getElementById("appTagline").textContent = profile.name ? `La liste perso de ${profile.name}` : "Ta liste de restaurants perso";
}
...
let welcomeColor  = "#F4813A";
let welcomeChoice = "empty"; // "empty" | "sample"

function initWelcomeScreen() {
  const profile = loadProfile();

  if (profile) {
    // Déjà configuré → masquer l'écran de bienvenue
    document.getElementById("welcomeScreen").classList.add("hidden");
    applyProfile(profile);
    return false;
  }

  // Premier lancement → afficher l'écran
  ...
  document.querySelectorAll("#colorPicker .color-dot").forEach(dot => {
    dot.addEventListener("click", () => {
      ...
      welcomeColor = dot.dataset.color;
      applyColor(welcomeColor);
    });
  });

  // List choice
  document.getElementById("choiceEmpty").addEventListener("click", () => {
    welcomeChoice = "empty";
    document.getElementById("choiceEmpty").classList.add("active");
    document.getElementById("choiceSample").classList.remove("active");
  });

  document.getElementById("choiceSample").addEventListener("click", () => {
    welcomeChoice = "sample";
    document.getElementById("choiceSample").classList.add("active");
    document.getElementById("choiceEmpty").classList.remove("active");
  });
  ...
  document.getElementById("welcomeStart").addEventListener("click", () => {
    ...
    const profile = { name, color: welcomeColor };
    saveProfile(profile);
    applyProfile(profile);

    // Si liste vierge → vider les données sample
    if (welcomeChoice === "empty") {
      restos = [];
      persist();
    }

    // Masquer l'écran avec animation
    const ws = document.getElementById("welcomeScreen");
    ws.style.transition = "opacity .4s, transform .4s";
    ws.style.opacity = "0";
    ws.style.transform = "scale(.97)";
    ...
    if (name) toast(`Bienvenue ${name} ! 🎉`);
    else      toast("C'est parti ! 🍽️");
  });
...
let settingsColor = "#F4813A";

function openSettings() {
  const profile = loadProfile() || { name: "", color: "#F4813A" };
  document.getElementById("settingsName").value = profile.name || "";
  settingsColor = profile.color || "#F4813A";

  document.querySelectorAll("#settingsColorPicker .color-dot").forEach(dot => {
    dot.classList.toggle("active", dot.dataset.color === settingsColor);
    dot.addEventListener("click", () => {
      document.querySelectorAll("#settingsColorPicker .color-dot").forEach(d => d.classList.remove("active"));
      dot.classList.add("active");
      settingsColor = dot.dataset.color;
      applyColor(settingsColor);
    });
  });
...
function saveSettings() {
  ...
  const profile = { name, color: settingsColor };
  saveProfile(profile);
  applyProfile(profile);
  closeSettings();
...
document.getElementById("btnResetData").addEventListener("click", () => {
  if (confirm("Effacer toute ta liste ? Cette action est irréversible.")) {
    restos = [];
    persist();
    closeSettings();
    renderStats();
...
const isFirstLaunch = initWelcomeScreen();
